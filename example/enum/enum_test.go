...
func Test_fromValue(t *testing.T) {
	val, found := fromValue("Blinky", true, "Ghost")
	assert.True(t, found)
	assert.False(t, val.IsUndefined())

	val, found = fromValue("bLINKY", false, "Ghost")
	assert.False(t, found)
	assert.True(t, val.IsUndefined())

	val, found = fromValue("Blinky", true, "invalid_8e0f038a-3281-401b-b750-4614942902b5")
	assert.False(t, found)
	assert.True(t, val.IsUndefined())
}
